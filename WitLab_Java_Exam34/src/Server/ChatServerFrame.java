...
public class ChatServerFrame extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(o==jbConn) {
			...
			LogWriter lw=new LogWriter(jtaLog, jspLog);
			ts = new TcpServer(port, lw);
			ts.start();
		} else if(o==jbSend) {
			ts.sendMsg("/msg Server : "+jtfContent.getText());
			jtaLog.append("Server : "+jtfContent.getText()+"\n");
			jspLog.getVerticalScrollBar().setValue(jspLog.getVerticalScrollBar().getMaximum());
			jtfContent.setText("");
		}
	}
}
